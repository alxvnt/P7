...
function display_wiki(quote, link)
// Create a div and display the wiki data
{
	var chat = document.getElementById('chat_box');
	var newDiv = document.createElement('div');
	newDiv.setAttribute("class", "chat_answer");
	...
	wiki_link.setAttribute("href", link);
	wiki_link.textContent = "En savoir plus"
	newDiv.appendChild(wiki_link);
	chat.appendChild(newDiv);
...
// Display all the data which come from the view
{
    ...
    if (dict == "1") {
        display_message("Désolé, ma mémoire me fait défaut");
    }
    else {
        display_message(tab[2]);
	    var latlng = new google.maps.LatLng(dict["lat"], dict["lng"]);
        initMap(latlng);
        display_wiki(tab[1], tab[3]);
    }
